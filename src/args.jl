...
const _styleAliases = Dict{Symbol,Symbol}(
    :a => :auto, :s => :solid, :d => :dash, :dd => :dashdot, :ddd => :dashdotdot
)
...
const _initial_fontsizes = merge(
    _initial_plt_fontsizes, _initial_sp_fontsizes, _initial_ax_fontsizes
)
...
const _all_args = sort(
    union([_all_axis_args; _all_subplot_args; _all_series_args; _all_plot_args])
)
...
add_aliases(
    :fill_z, :fillz, :fz, :surfacecolor, :surfacecolour, :sc, :surfcolor, :surfcolour
)
...
function RecipesPipeline.preprocess_attributes!(plotattributes::AKW)
    ...
    if haskey(plotattributes, :series_annotations)
        plotattributes[:series_annotations] = series_annotations(
            wraptuple(plotattributes[:series_annotations])...
        )
    end
...
function slice_arg!(
    plotattributes_in, plotattributes_out, k::Symbol, idx::Int, remove_pair::Bool
)
...
function _update_axis(
    plt::Plot, sp::Subplot, plotattributes_in::AKW, letter::Symbol, subplot_index::Int
)
...
function _update_axis(
    axis::Axis, plotattributes_in::AKW, letter::Symbol, subplot_index::Int
)
...
function _update_subplot_args(
    plt::Plot, sp::Subplot, plotattributes_in, subplot_index::Int, remove_pair::Bool
)
...
function _update_series_attributes!(plotattributes::AKW, plt::Plot, sp::Subplot)
    ...
    aliasesAndAutopick(
        plotattributes, :linestyle, _styleAliases, supported_styles(pkg), plotIndex
    )
    aliasesAndAutopick(
        plotattributes, :markershape, _markerAliases, supported_markers(pkg), plotIndex
    )
    ...
    for s in (:line, :marker, :fill)
        ...
        plotattributes[csym] = if plotattributes[csym] == :auto
            plot_color(
                if has_black_border_for_default(stype) && s == :line
...
macro add_attributes(level, expr)
    ...
    for (key, value) in zip(key_args, value_args)
        ...
        push!(
            insert_block.args, :(add_aliases($(QuoteNode(exp_key)), $(QuoteNode(pl_key))))
        )
        push!(
            insert_block.args,
            :(add_aliases(
                $(QuoteNode(exp_key)), $(QuoteNode(make_non_underscore(exp_key)))
            )),
        )
        push!(
            insert_block.args,
            :(add_aliases(
                $(QuoteNode(exp_key)), $(QuoteNode(make_non_underscore(pl_key)))
            )),
        )
    end
    return esc(
