...
const _label_func = Dict{Symbol,Function}(
    :log10 => x -> "10^$x", :log2 => x -> "2^$x", :ln => x -> "e^$x"
)
...
function optimal_ticks_and_labels(ticks, alims, scale, formatter)
    ...
    if ticks === nothing && scale == :identity
        if formatter == RecipesPipeline.dateformatter
            ...
            ticks, labels = optimize_datetime_ticks(
                864e5 * amin, 864e5 * amax; k_min=2, k_max=4
            )
...
function expand_extrema!(sp::Subplot, plotattributes::AKW)
    ...
    for letter in (:x, :y, :z)
        ...
        elseif data !== nothing
            ...
            plotattributes[letter], plotattributes[Symbol(letter, "_discrete_indices")] = discrete_value!(
                axis, data
            )
    ...
    if sp[:framestyle] != :none
        ...
        if ax[:ticks] ∉ (:none, nothing, false)
            ...
            if ax[:minorticks] ∉ (:none, nothing, false) || ax[:minorgrid]
                add_major_or_minor_segments(
                    minor_ticks, ax[:minorgrid], minorgrid_segments, 0.6 / ax_length, true
                )
            end
        end
    end
...
function axis_drawing_info_3d(sp, letter)
    ...
    if sp[:framestyle] != :none# && letter === :x
        ...
        if ax[:ticks] ∉ (:none, nothing, false)
            ...
            # add major grid segments
            add_major_or_minor_segments(
                ticks[1], ax[:grid], grid_segments, 0.012, ax[:tick_direction] !== :none
            )

            # add minor grid segments
            if ax[:minorticks] ∉ (:none, nothing, false) || ax[:minorgrid]
                add_major_or_minor_segments(
                    minor_ticks, ax[:minorgrid], minorgrid_segments, 0.006, true
                )
            end
        end
    end
