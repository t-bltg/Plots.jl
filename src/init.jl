...
function __init__()
    ...
    insert!(
        ...
        findlast(
            x -> x isa Base.TextDisplay || x isa REPL.REPLDisplay, Base.Multimedia.displays
        ) + 1,
        PlotsDisplay(),
    )
    ...
    if get(ENV, "PLOTS_HOST_DEPENDENCY_LOCAL", "false") == "true"
        global plotly_local_file_path[] = joinpath(
            @get_scratch!("plotly"), _plotly_min_js_filename
        )
        if !isfile(plotly_local_file_path[])
            download(
                "https://cdn.plot.ly/$(_plotly_min_js_filename)", plotly_local_file_path[]
            )
        end
