...
function heatmap_edges(
    v::AVec, scale::Symbol=:identity, isedges::Bool=false, ispolar::Bool=false
)
...
function dumpdict(io::IO, plotattributes::AKW, prefix="", alwaysshow=false)
    _debugMode.on || alwaysshow || return nothing
    println(io)
    if prefix != ""
        println(io, prefix, ":")
    end
...
function straightline_data(series, expansion_factor=1)
    ...
    xdata, ydata = if n == 2
        ...
        if r == 0
            ...
            for i in 1:k
                inds = (3 * i - 2):(3 * i - 1)
                xdata[inds], ydata[inds] = straightline_data(
                    xl, yl, x[inds], y[inds], expansion_factor
                )
            end
            xdata, ydata
        else
