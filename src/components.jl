...
function makecross(; offset=-0.5, radius=1.0)
    ...
    Shape(
        weave(
            outercircle, innercircle; ordering=Vector[outercircle, innercircle, outercircle]
        ),
    )
end
...
function series_annotations_shapes!(series::Series, scaletype::Symbol=:pixels)
    ...
    if anns !== nothing && anns.baseshape !== nothing
        ...
        for i in eachindex(anns.strs)
            ...
            shapes[i] = scale(
                baseshape, msw * xscale / maxscale, msh * yscale / maxscale, (0, 0)
            )
        end
...
function process_annotation(
    sp::Subplot, positions::Union{AVec{Symbol},Symbol,Tuple}, labs, font=_annotationfont(sp)
)
    anns = []
