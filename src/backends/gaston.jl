...
for (mime, term) in (
    ...
    @eval function _show(io::IO, ::MIME{Symbol($mime)}, plt::Plot{GastonBackend})
        ...
        Gaston.save(;
            term=term, output=tmpfile, handle=plt.o.handle, saveopts=gaston_saveopts(plt)
        )
...
function gaston_saveopts(plt::Plot{GastonBackend})
    ...
    push!(
        saveopts,
        gaston_font(plottitlefont(plt); rot=false, align=false, color=false, scale=1),
    )
...
function gaston_init_subplot(
    plt::Plot{GastonBackend}, sp::Union{Nothing,Subplot{GastonBackend}}
)
    if sp === nothing
...
function gaston_seriesconf!(
    sp::Subplot{GastonBackend}, series::Series, i::Int, add_to_legend::Bool
)
    #=
...
function gaston_parse_axes_args(
    ...
    for letter in (:x, :y, :z)
        ...
        # label names
        push!(
            axesconf, "set $(letter)label '$(axis[:guide])' $(gaston_font(guidefont(axis)))"
        )
...
function gaston_set_legend!(axesconf, sp, any_label)
    ...
    if sp[:legend] ∉ (:none, :inline) && any_label
        ...
        push!(
            axesconf, "set key " * (occursin("outer", string(leg)) ? "outside" : "inside")
        )
