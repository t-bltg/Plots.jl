...
function plotly_axis(axis, sp, anchor=nothing, domain=nothing)
    ...
    ax = KW(
        ...
        :ticks => if axis[:tick_direction] === :out
            "outside"
        elseif axis[:tick_direction] === :in
            "inside"
        else
            ""
        end,
...
function plotly_layout(plt::Plot)
    ...
    for sp in plt.subplots
        ...
        if ispolar(sp)
            plotattributes_out[Symbol("angularaxis$(spidx)")] = plotly_polaraxis(
                sp, sp[:xaxis]
            )
            plotattributes_out[Symbol("radialaxis$(spidx)")] = plotly_polaraxis(
                sp, sp[:yaxis]
            )
        else
            ...
            if RecipesPipeline.is3d(sp)
                ...
                plotattributes_out[Symbol("xaxis$(x_idx)")] = plotly_axis(
                    sp[:xaxis], sp, string("y", y_idx), x_domain
                )
                # don't allow yaxis to be reupdated/reanchored in a linked subplot
                if spidx == y_idx
                    plotattributes_out[Symbol("yaxis$(y_idx)")] = plotly_axis(
                        sp[:yaxis], sp, string("x", x_idx), y_domain
                    )
                end
            end
        end
        ...
        for ann in sp[:annotations]
            append!(
                ...
                KW[plotly_annotation_dict(
                    locate_annotation(sp, ann...)...; xref="x$(x_idx)", yref="y$(y_idx)"
                )],
            )
        end
        # series_annotations
            ...
            for (xi, yi, str, fnt) in EachAnn(anns, series[:x], series[:y])
                push!(
                    ...
                    plotly_annotation_dict(
                        xi, yi, PlotText(str, fnt); xref="x$(x_idx)", yref="y$(y_idx)"
                    ),
                )
            end
        end
...
function plotly_legend_pos(pos::Symbol)
    ...
    legend_position = get(
        plotly_legend_position_mapping, pos, plotly_legend_position_mapping.default
    )
end
...
function plotly_series(plt::Plot, series::Series)
    ...
    elseif st == :heatmap
        ...
        plotattributes_out[:colorscale] = plotly_colorscale(
            series[:fillcolor], series[:fillalpha]
        )
    ...
    elseif st == :contour
        ...
        plotattributes_out[:colorscale] = plotly_colorscale(
            series[:linecolor], series[:linealpha]
        )
    ...
    elseif st in (:surface, :wireframe)
        ...
        if st == :wireframe
            ...
            plotattributes_out[:contours] = KW(
                :x => wirelines, :y => wirelines, :z => wirelines
            )
    ...
    elseif st == :mesh3d
        ...
        if series[:connections] !== nothing
            if typeof(series[:connections]) <: Tuple{Array,Array,Array}
                ...
                throw(
                    ArgumentError("Argument connections has to be a tuple of three arrays.")
                )
            end
        end
        plotattributes_out[:colorscale] = plotly_colorscale(
        ...
        plotattributes_out[:color] = rgba_string(
            plot_color(series[:fillcolor], series[:fillalpha])
        )
        plotattributes_out[:opacity] = series[:fillalpha]
        if series[:fill_z] !== nothing
            plotattributes_out[:surfacecolor] = handle_surface(series[:fill_z])
    ...
    if hasmarker
        ...
        plotattributes_out[:marker] = KW(
            ...
            :color =>
                rgba_string.(
                    plot_color.(
                        get_markercolor.(series, inds), get_markeralpha.(series, inds)
                    ),
                ),
            :line => KW(
                :color =>
...
function plotly_series_shapes(plt::Plot, series::Series, clims)
    ...
    elseif series[:marker_z] !== nothing
        push!(
            plotattributes_outs, plotly_colorbar_hack(series, plotattributes_base, :marker)
        )
    end
    plotattributes_outs
end
...
function plotly_series_segments(series::Series, plotattributes_base::KW, x, y, z, clims)
    ...
    for (k, segment) in enumerate(segments)
        ...
        elseif st in (:path3d, :scatter3d)
            ...
            plotattributes_out[:x], plotattributes_out[:y], plotattributes_out[:z] = x[rng],
            y[rng],
            z[rng]
        end
        ...
        if hasmarker
            ...
            lcolor = rgba_string(
                plot_color(
                    get_markerstrokecolor(series, i), get_markerstrokealpha(series, i)
                ),
            )
            plotattributes_out[:marker] = KW(
        ...
        if hasfillrange
            ...
            if isa(series[:fillrange], AbstractVector)
                ...
                plotattributes_out_fillrange[:x], plotattributes_out_fillrange[:y] = concatenate_fillrange(
                    x[rng], series[:fillrange]
                )
