...
function _inspectdr_add_annotations(plot, x, y, val::PlotText)
    ...
    fnt = InspectDR.Font(
        val.font.family, val.font.pointsize; color=_inspectdr_mapcolor(val.font.color)
    )
    ann = InspectDR.atext(val.str; x=x, y=y, font=fnt, angle=val.font.rotation, align=align)
    ...
    if st in (:shape,)
        ...
        if i > 1 #Add dummy waveform for legend entry:
            ...
            wfrm.glyph = InspectDR.glyph(;
                shape=INSPECTDR_GLYPH_SHAPE, size=8, color=linecolor, fillcolor=fillcolor
            )
        end
