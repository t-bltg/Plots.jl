            ...
            if series[:fillrange] !== nothing && st != :shape
                push!(
                    ...
                    pgf_fillrange_series(
                        series, i, _cycle(series[:fillrange], rng), seg_args...
                    ),
                )
            end
...
function pgf_axis(sp::Subplot, letter)
    ...
    if !(axis[:ticks] in (nothing, false, :none, :native)) && framestyle != :none
        ...
        if axis[:showaxis] && axis[:scale] in (:ln, :log2, :log10) && axis[:ticks] == :auto
            ...
            push!(
                style, string(letter, "ticklabels = {\$", join(tick_labels, "\$,\$"), "\$}")
            )
    ...
    if framestyle == :zerolines
        ...
        push!(
            ...
            string(
                ...
                pgf_linestyle(
                    pgf_thickness_scaling(sp), axis[:foreground_color_border], 1.0
                ),
                "}}",
            ),
        )
...
""",
        ...
        for series in series_list(sp)
            ...
            for (xi, yi, str, fnt) in EachAnn(anns, series[:x], series[:y])
                pgf_add_annotation!(
                    o, xi, yi, PlotText(str, fnt), pgf_thickness_scaling(series)
                )
            end
        end
        ...
        for ann in sp[:annotations]
            pgf_add_annotation!(
                o, locate_annotation(sp, ann...)..., pgf_thickness_scaling(sp)
            )
        end
...
function _show(io::IO, mime::MIME"application/x-tex", plt::Plot{PGFPlotsBackend})
    fn = tempname() * ".tex"
    PGFPlots.save(
        fn, backend_object(plt); include_preamble=plt.attr[:tex_output_standalone]
    )
    write(io, read(open(fn), String))
end
