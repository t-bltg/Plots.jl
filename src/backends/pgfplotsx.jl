    ...
    if !pgfx_plot.is_created || pgfx_plot.was_shown
        ...
        for sp in plt.subplots
            ...
            if sp[:legendtitle] !== nothing
                ...
                push!(
                    ...
                    PGFPlotsX.LegendEntry(
                        string("\\hspace{-.6cm}{\\textbf{", sp[:legendtitle], "}}"), false
                    ),
                )
            end
            for (series_index, series) in enumerate(series_list(sp))
                ...
                extra_series, extra_series_opt = pgfx_split_extra_opts(
                    series[:extra_kwargs]
                )
                ...
                for (xi, yi, str, fnt) in EachAnn(anns, series[:x], series[:y])
                    pgfx_add_annotation!(
                        axis, xi, yi, PlotText(str, fnt), pgfx_thickness_scaling(series)
                    )
                end
            end # for series
            ...
            elseif sf isa Tuple && series[:ribbon] !== nothing
                for sfi in sf
                    pgfx_fillrange_series!(
                        axis, series, series_func, i, _cycle(sfi, rng), rng
                    )
                end
            end
            if k == 1 &&
...
function pgfx_add_series!(::Val{:mesh3d}, axis, series_opt, series, series_func, opt)
    ...
    push!(
        series_opt, "patch" => nothing, "table/row sep" => "\\\\", "patch table" => ptable
    )
    pgfx_add_series!(axis, series_opt, series, series_func, opt)
end
...
function pgfx_add_series!(::Val{:contour3d}, axis, series_opt, series, series_func, opt)
    ...
    series_plot = series_func(
        series_opt, PGFPlotsX.Table(Contour.contours(args..., opt[:levels]))
    )
    push!(axis, series_plot)
    pgfx_add_legend!(axis, series, opt)
end
...
function pgfx_add_series!(::Val{:quiver}, axis, series_opt, series, series_func, opt)
    if opt[:quiver] !== nothing
        ...
        if z !== nothing
            ...
            table = PGFPlotsX.Table([
                :x => x, :y => y, :u => opt[:quiver][1], :v => opt[:quiver][2]
            ])
        end
    end
...
function pgfx_get_legend_style(sp)
    ...
    PGFPlotsX.Options(
        pgfx_linestyle(
            pgfx_thickness_scaling(sp), sp[:foreground_color_legend], fg_alpha, "solid"
        ) => nothing,
        "fill" => cstr,
        "fill opacity" => a,
...
function pgfx_marker(plotattributes, i=1)
    ...
    cstr = plot_color(
        get_markercolor(plotattributes, i), get_markeralpha(plotattributes, i)
    )
    a = alpha(cstr)
    cstr_stroke = plot_color(
        get_markerstrokecolor(plotattributes, i), get_markerstrokealpha(plotattributes, i)
    )
        ...
        join([
            ...
            sprint(
                ...
                merge(
                    PGFPlotsX.Options(
                        get(
                            (hcenter="", left="right", right="left"), val.font.halign, ""
                        ) => nothing,
                        "color" => cstr,
...
function pgfx_axis!(opt::PGFPlotsX.Options, sp::Subplot, letter)
    ...
    if framestyle in (:axes, :origin)
        axispos = if axis[:mirror]
            "right"
        elseif framestyle == :axes
            "left"
        else
            "middle"
        end
    ...
    if framestyle == :zerolines
        ...
        push!(
            ...
            string("extra ", letter, " tick style") => PGFPlotsX.Options(
                "grid" => "major",
                string(letter, " grid style") => pgfx_linestyle(
                    pgfx_thickness_scaling(sp), axis[:foreground_color_border], 1.0
                ),
            ),
        )
    end
...
function _show(io::IO, mime::MIME{Symbol("application/x-tex")}, plt::Plot{PGFPlotsXBackend})
    plt.o.was_shown = true
    PGFPlotsX.print_tex(
        io, plt.o.the_plot; include_preamble=plt.attr[:tex_output_standalone]
    )
end
