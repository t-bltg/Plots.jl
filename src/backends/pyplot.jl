...
function py_add_series(plt::Plot{PyPlotBackend}, series::Series)
                ...
                if typeof(a) != Arrow
                    ...
                    add_arrows(x, y) do xyprev, xy
                        ax."annotate"(
                            "";
                            xytext=(
                                0.001xyprev[1] + 0.999xy[1], 0.001xyprev[2] + 0.999xy[2]
                            ),
                            xy=xy,
    ...
    if st == :hexbin
        ...
        extrakw[:edgecolors] = get(
            series[:extra_kwargs], :edgecolors, py_color(get_linecolor(series))
        )
        handle = ax."hexbin"(
            x,
            y;
            ...
            if st == :surface
                if series[:fill_z] !== nothing
                    ...
                    extrakw[:facecolors] = py_shading(
                        series[:fillcolor], py_handle_surface(series[:fill_z])
                    )
                    extrakw[:shade] = false
                else
        ...
        for segment in series_segments(series)
            ...
            if length(rng) > 1
                ...
                patches = pypatches."PathPatch"(
                    ...
                    zorder=series[:series_plotindex],
                    edgecolor=py_color(
                        get_linecolor(series, clims, i), get_linealpha(series, i)
                    ),
                    facecolor=py_color(
                        get_fillcolor(series, clims, i), get_fillalpha(series, i)
                    ),
                    linewidth=py_thickness_scale(plt, get_linewidth(series, i)),
                    linestyle=py_linestyle(st, get_linestyle(series, i)),
                    fill=true,
...
function py_set_scale(ax, sp::Subplot, scale::Symbol, letter::Symbol)
    ...
    arg = if scale == :identity
        ...
        kw[Symbol(:linthresh, pyletter)] = NaNMath.max(
            1e-16, py_compute_axis_minval(sp, axis)
        )
        "symlog"
    end
    func(arg; kw...)
    ...
    for sp in plt.subplots
        ...
        if hascolorbar(sp)
            ...
            if RecipesPipeline.is3d(sp) || ispolar(sp)
                cbax = fig."add_axes"(
                    [0.9, 0.1, 0.03, 0.8]; label=string("cbar", sp[:subplot_index])
                )
                ...
                cb = fig."colorbar"(
                    handle; cax=cbax, orientation=colorbar_orientation, kw...
                )
        ...
        for letter in (:x, :y, :z)
            ...
            if RecipesPipeline.is3d(sp)
                ...
                getproperty(ax, Symbol("set_", letter, "ticklabels"))(
                    positions; fontdict=fontProperties
                )
            end
                ...
                push!(
                    ...
                    if series[:seriestype] == :shape || series[:fillrange] !== nothing
                        pypatches."Patch"(;
                            ...
                            linewidth=py_thickness_scale(
                                plt, clamp(get_linewidth(series), 0, 5)
                            ),
                            linestyle=py_linestyle(
                                series[:seriestype], get_linestyle(series)
                            ),
                            capstyle="butt",
                        )
                    elseif series[:seriestype] in
                        ...
                        PyPlot.plt."Line2D"(
                            ...
                            linewidth=py_thickness_scale(
                                plt, hasline * sp[:legendfontsize] / 8
                            ),
